...
pub(crate) fn day_four_part_two(input: &str) -> u32 {
    let total_games = input.lines().count();
    let mut total_cards = vec![1;total_games];

    for game_string in input.lines() {
        let game_card_tuple = split_game_card(game_string);
        let wins = count_game_wins(&game_card_tuple);

        let games_to_play = total_cards[(game_card_tuple.0-1) as usize];
        ...
        for _ in 0..games_to_play {
            ...
            for k in 0..wins {
                total_cards[(game_card_tuple.0 + k) as usize] += 1;
            }
        }
    }
...
fn count_game_wins(game_card_tuple: &(u32, Vec<u32>, Vec<u32>)) -> u32 {

    let (_, card_numbers, winning_numbers) = game_card_tuple;

    card_numbers.iter().filter_map(|n| {
        if winning_numbers.contains(n) {
            Some(n)
        } else {
            None
...
mod tests {
    ...
    fn test_day_four_part_two() {
        ...
        assert_eq!(day_four_part_two(input), 30);
    }
}